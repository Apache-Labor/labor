...
if ( $params[:do_boundaries] )
  ...
  0.upto($params[:num_bins]) do |i|
    bins[i] = [i, $params[:min] + i * step, 0]
  end

  # count individual values and put it inside the bin array
  values.each do |item|
    target_bin = (item - $params[:min]) / step
    begin
      bins[target_bin][2] += 1 # raise number of occ. of this bin
    rescue
      nil
    end
  end

  if bins[$params[:num_bins]][2] == 1
	# add the last single item to the bin before and remove that last bin
	# this results in the correct number of bins
  	bins[$params[:num_bins]-1][2] = bins[$params[:num_bins]-1][2] + 1
  	bins.delete_at($params[:num_bins])
  end
