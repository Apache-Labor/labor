...
Für den Moment lassen wir das unberührt und nehmen aber wie erwähnt drei entscheidende Werte aus `crs-setup.conf` heraus und definieren sie in unserer Konfiguration, damit wir sie dauerhaft in Blick behalten können. Zunächst setzen wir zwei Limiten in der Regel _900110_: Den Anomalie-Wert der Anfragen und den Anomalie-Wert der Antworten. Dies geschieht mittels der `setvar` Action, welche beide Werte auf 1'000 setzt.
...
Darauf folgenden die Regeln, die sich um die Antworten kümmern. Sie sind geringer in der Anzahl und suchen prinzipiell nach Code Lecks (Stack Traces!) und Lecks in Fehlermeldungen (die einem Angreifer sehr dabei helfen, eine SQL Attacke zu konstruieren). Der Anomalie-Wert der Antworten wird im Regel-File mit dem 959er Prefix überprüft.
